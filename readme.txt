...
= 0.6.0 =
* New: an error is attributed to the component that actually shipped its file — and to nobody, when nobody did. Every report names the component the file belongs to, and the fallback used to be `core` for every path outside a plugin or theme root: a PHP error thrown from `wp-content/uploads/2026/09/x.php` was filed as an error in WordPress itself. A dropped webshell is exactly the file that throws once and never again, and `core` is the one label that makes it invisible. The vocabulary is closed now and says what it knows: `plugin:<slug>`, `mu-plugin:<slug>`, `theme:<slug>`, `core` (wp-admin/, wp-includes/ or one of core's own root files), `dropin` (a `wp-content/` drop-in WordPress loads by name, so a caching plugin's `advanced-cache.php` does not read as a stranger), `uploads` (where no PHP belongs) and `unknown` — under the site and shipped by nobody, which is the interesting one. Uploads are resolved through `wp_get_upload_dir()`, the variant that does not create the directory as a side effect: an error path must never write to disk. No new sensor, no scan, no setting — the same error stream, told the truth. For a site with no repository mapped in the console, this is what stands in for the deploy-manifest verdict.
